...
VINOInference::VINOInference(int model_index, std::string cpu_threads, bool force_cpu_mode)
{
	// logger
	...
	if (gpu_mode)
	{
		model_folder = kModelDir_ + "/2"; // force to use the best model if gpu detected
	}
	else
	{
		model_folder = kModelDir_ + '/' + std::to_string(model_index);
	}
